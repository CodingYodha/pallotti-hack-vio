...
async def upload_video(
    ...
    mail_to: Optional[str] = Form(None),
    ...
    background_tasks.add_task(process_video_background, video.id, file_path, send_email, mail_to)
...
async def mark_video_reviewed(
    ...
    # Only auto-confirm PENDING violations (preserve user's rejected decisions)
    ...
    if individual_ids:
        # Only update PENDING violations - preserve confirmed/rejected decisions
        update_stmt = (
            update(Violation)
            .where(Violation.individual_id.in_(individual_ids))
            .where(Violation.review_status == "pending")  # Only pending!
            .values(
