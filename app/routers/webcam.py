    ...
    try:
        while True:
            ...
            try:
                ...
                # Build employee names map from face service
                employee_names = {}
                unknown_snapshots = {}
                if pipeline.face_service:
                    for tid_key, (emp_id, emp_name) in pipeline.face_service.track_id_to_employee.items():
                        if emp_name:
                            employee_names[str(tid_key)] = emp_name
                        else:
                            employee_names[str(tid_key)] = f"Unknown-{tid_key}"
                    unknown_snapshots = {str(k): v for k, v in pipeline.face_service.unknown_snapshots.items()}
                ...
                await websocket.send_json({
                    ...
                    "employee_names": employee_names,  # Employee names by track_id
                    "unknown_snapshots": unknown_snapshots  # Snapshots of unknown persons
                })
...
async def save_webcam_session(
    ...
    try:
        ...
        for ind in request.individuals:
            ...
            # Flaw 6 fix: look up the employee matched by the face service for this
            # track_id and persist it so the dashboard shows the real name.
            pipeline = get_webcam_pipeline()
            emp_id = None
            emp_name = None
            if pipeline.face_service:
                emp_id, emp_name = pipeline.face_service.track_id_to_employee.get(
                    ind.person_id, (None, None)
                )
            ...
            tracked_ind = TrackedIndividual(
                ...
                employee_id=emp_id,    # Flaw 6 fix: persist matched employee
