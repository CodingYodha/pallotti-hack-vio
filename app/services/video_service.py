...
async def process_video_background(
    ...
    async with async_session_maker() as db:
        try:
            ...
            # Send batch report if requested
            if send_email and mail_to:
                # We need to construct profiles from the processing result 
                # (which was already saved to DB but is returned by process_video_sync)
                profiles = processing_result.individual_profiles
...
async def save_processing_results(
    ...
    # Get employee mapping from face recognition (if available)
    employee_mapping = result.employee_mapping or {} if hasattr(result, 'employee_mapping') else {}
    unknown_snapshots = result.unknown_snapshots or {} if hasattr(result, 'unknown_snapshots') else {}
    ...
    for profile in result.individual_profiles.values():
        ...
        # Get employee info if face was recognized
        emp_info = employee_mapping.get(profile.track_id, {})
        emp_id = emp_info.get('employee_id') if emp_info else None
        emp_name = emp_info.get('employee_name') if emp_info else None
        
        # Get face snapshot path (for unknown persons)
        snap_path = unknown_snapshots.get(profile.track_id, None)
