        ...
        <>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {personViolation?.image_path ? (
                                                <img
                                                    src={getImageUrl(personViolation.image_path)}
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2">
                                                    <span className="font-semibold">
                                                        {ind.employee_name || `Unknown-${ind.track_id}`}
                                                    </span>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-3">
                                            <h2 className="text-lg font-semibold">
                                                {selectedIndividual.employee_name || `Unknown-${selectedIndividual.track_id}`}
                                            </h2>
                                            {!selectedIndividual.employee_name && (
                                                <button
                                                    className="btn btn-ghost btn-sm"
                                                    onClick={() => {
                                                        const snapshot = violations.find(v => v.track_id === selectedIndividual.track_id && v.image_path)
                                                        setNamingPerson({
                                                            trackId: selectedIndividual.track_id,
                                                            snapshotPath: selectedIndividual.snapshot_path || snapshot?.image_path
                                                        })
                                                    }}
                                                    title="Name this person"
                                                >
                                                    <UserPlus size={14} /> Name
                                                </button>
                                            )}
                                        </div>
            ...
            {namingPerson && (
                <NamePersonModal
                    ...
                    onSave={(name) => {
                        // Update individuals list with the new name
                        setIndividuals(prev => prev.map(ind =>
                            ind.track_id === namingPerson.trackId
                                ? { ...ind, employee_name: name }
                                : ind
                        ))
                        if (selectedIndividual && selectedIndividual.track_id === namingPerson.trackId) {
                            setSelectedIndividual(prev => ({ ...prev, employee_name: name }))
                        }
