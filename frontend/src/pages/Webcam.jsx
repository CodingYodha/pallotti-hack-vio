...
function Webcam() {
    ...
    const employeeNamesRef = useRef({}) // track_id -> name
    const unknownSnapshotsRef = useRef({}) // track_id -> snapshot_path

    // Employee identification
    const [employeeNames, setEmployeeNames] = useState({}) // track_id -> name
    const [unknownSnapshots, setUnknownSnapshots] = useState({}) // track_id -> snapshot_path
        ...
        try {
            ...
            ws.onmessage = (event) => {
                ...
                if (data.employee_names) {
                    employeeNamesRef.current = data.employee_names
                }
                if (data.unknown_snapshots) {
                    unknownSnapshotsRef.current = data.unknown_snapshots
                }
    ...
    const stopStreaming = () => {
        ...
        // Save PPE data and employee names
        setPersonPpe(ppeRef.current)
        setEmployeeNames(employeeNamesRef.current)
        setUnknownSnapshots(unknownSnapshotsRef.current)
        cleanup()
        setSessionEnded(true)
    }
                        ...
                        >
                            ...
                            <div className="card-body" style={{ padding: '1rem' }}>
                                ...
                                <div className="text-sm text-muted mb-3">
                                    <div>Person: {employeeNames[String(violation.person_id)] || `Unknown-${violation.person_id}`}</div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2">
                                            <span className="font-semibold">{employeeNames[String(ind.person_id)] || `Unknown-${ind.person_id}`}</span>
                ...
                {!selectedIndividual ? (
                    ...
                    <div className="card-body">
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h2 className="text-lg font-semibold">{employeeNames[String(selectedIndividual.person_id)] || `Unknown-${selectedIndividual.person_id}`}</h2>
                        ...
                        {/* Name This Person button for unknown individuals */}
                        {(!employeeNames[String(selectedIndividual.person_id)] || employeeNames[String(selectedIndividual.person_id)]?.startsWith('Unknown')) && (
                            <button
                                className="btn btn-secondary mt-4"
                                style={{ width: '100%' }}
                                onClick={() => setNamingPerson({
                                    trackId: selectedIndividual.person_id,
                                    snapshotPath: unknownSnapshots[String(selectedIndividual.person_id)] || selectedIndividual.image_path
                                })}
                            >
                                <User size={16} />
                                Name This Person
    ...
    return (
        <div className="webcam-page">
                                ...
                                {stats.recent_violations.length === 0 ? (
                                    ...
                                    stats.recent_violations.map((v, idx) => (
                                        <div key={idx} className="violation-item">
                                            ...
                                            <span className="violation-person">{employeeNamesRef.current[String(v.person_id)] || `Unknown-${v.person_id}`}</span>
            ...
            {namingPerson && (
                <NamePersonModal
                    snapshotPath={namingPerson.snapshotPath}
                    trackId={namingPerson.trackId}
                    onClose={() => setNamingPerson(null)}
                    onSave={(name) => {
                        setEmployeeNames(prev => ({ ...prev, [String(namingPerson.trackId)]: name }))
